...
// ===========================================================================
// P2B-A Preview Workbench — 后期制作脑子预览（Preview Only，不执行视频）
// ===========================================================================

// ---- P2B-A 类型定义 ----

export interface ThemeKernel {
  core_message?: string;
  ...
  brand_memory_point?: string;
  [key: string]: unknown;
}

export interface ExecutionPlanSkillChainItem {
  skill_id: string;
  display_name?: string;
  params?: Record<string, unknown>;
}
...
export interface ExecutionPlan {
  ...
  highlight_focus_cn?: string;
  visual_style_cn?: string;
  skill_chain?: (ExecutionPlanSkillChainItem | string)[];
  cost_estimate?: number | string | null;
  execute_allowed?: boolean | number | null;
  craft_explanation?: string;
  rhythm_plan?: string;
  transition_plan?: string;
  subtitle_plan?: string;
  highlight_card_plan?: string;
  uniqueness_plan?: string;
  cta_plan?: string;
  variant_plan_json?: string | Record<string, unknown> | null;
  status?: string;
...
export interface ExecutionPlanExplain {
  execution_plan_id?: string;
  variant_id?: string;
  theme_core_message?: string;
  rhythm_explanation?: string;
  transition_explanation?: string;
  subtitle_explanation?: string;
  highlight_card_explanation?: string;
  uniqueness_explanation?: string;
  craft_explanation?: string;
  [key: string]: unknown;
}

export interface P2BSkillItem {
  skill_id: string;
  name: string;
  category?: string;
  layer?: string;
...
// ---- P2B-A API 函数（Preview Only，不执行视频）----

/** P2B API 1: GET /production-orders/{id}（复用 P2A 已有） */
// getProductionOrder 已在 P2A 定义，P2B 复用

/** P2B API 2: POST /p2b/theme-kernels */
export const p2bThemeKernels = (productionOrderId: string) =>
  post<ThemeKernel>("/p2b/theme-kernels", {
    production_order_id: productionOrderId,
  });

/** P2B API 3: POST /p2b/execution-plans/preview */
export const p2bExecutionPlansPreview = (
  productionOrderId: string, fissionPlanId: string | null = null,
) =>
  post<ExecutionPlanPreviewResult>("/p2b/execution-plans/preview", {
    production_order_id: productionOrderId,
    fission_plan_id: fissionPlanId,
  });

/** P2B API 4: POST /p2b/execution-plans（确认入库） */
export const p2bExecutionPlansConfirm = (
  productionOrderId: string, fissionPlanId: string | null = null,
) =>
  post<ExecutionPlanConfirmResult>("/p2b/execution-plans", {
    production_order_id: productionOrderId,
    fission_plan_id: fissionPlanId,
  });

/** P2B API 5: GET /p2b/execution-plans/{id} */
export const p2bGetExecutionPlan = (id: string) =>
  get<ExecutionPlan>(`/p2b/execution-plans/${id}`);

/** P2B API 6: GET /p2b/execution-plans/{id}/explain */
export const p2bExplainExecutionPlan = (id: string) =>
  get<ExecutionPlanExplain>(`/p2b/execution-plans/${id}/explain`);
...
/** P2B API 8: GET /p2b/skills */
export const p2bListSkills = () =>
  get<{ items?: P2BSkillItem[]; total?: number }>("/p2b/skills");
