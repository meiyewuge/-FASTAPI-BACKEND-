...
选择母视频
  ...
  → b_engine（商业内容生成器）：
      切片 → 重组 → 套用「内容策略」：
        · 策略分型：引流型 / 成交型 / IP型 / 招商型 / 获客型（mix 轮换或指定）
        · 情绪结构：痛点开头 → 冲突推进 → 解决方案 → 行动引导
        · 门店差异化：同一母视频 → 各门店/城市版本（南京版/广州版…），并归因 store_id
  → 输出 10~50 条裂变视频
  → 批量写入 models（带 tenant_id + store_id） + 生成分发链接（多账号矩阵）
```

> 策略库见 `backend/b_engine/strategies.py`（模板驱动、零 LLM）。
