<template>
  <div class="page">
    ...
    <template v-else-if="data">
      ...
      <div class="btn-row">
        <!-- P0B-2: 趋势页链接携带 token -->
        <van-button block round type="success" :to="trendsPath">查看趋势</van-button>
      </div>
      <div class="btn-row">
...
onMounted(async () => {
  try {
    ...
    const res = await api.get(`/api/monthly-checkups/${route.params.id}`, { params })
    data.value = res.data.data

    // P0B-2: ticket 鉴权成功后，后端返回 access_token → 升级 URL
    if (ticket && res.data.data.access_token) {
      router.replace(`/monthly/result/${route.params.id}?token=${encodeURIComponent(res.data.data.access_token)}`)
    }
  } catch (e: any) {
