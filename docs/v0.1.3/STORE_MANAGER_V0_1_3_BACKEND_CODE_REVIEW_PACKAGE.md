...
> **口径定死（阈值小提交阶段）**：① 基础 `68263a1`；首轮初审修订 HEAD `f1f29a4`；② **本次复审对象代码 HEAD = `2949888`**（复审只到此为止的代码）；③ **`2949888` 之后仅 docs-only 提交**，不改业务代码；④ `GIT_LOG.txt` 逐行 `[CODE]/[DOCS]` 标注，`2949888` 标为 ★代码HEAD。三处口径一致：代码看 `2949888`。
>
> **审查材料留存边界**：本审查包（docx / zip / patch / smoke 等）仅 review 阶段保留；**merge 前必须移除，不允许合入主分支**。
