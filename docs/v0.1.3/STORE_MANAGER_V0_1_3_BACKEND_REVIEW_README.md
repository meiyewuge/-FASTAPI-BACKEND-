...
| **代码 HEAD（本次复审对象）** | **`2949888`** | **最后一个含代码改动的提交**（阈值小提交 4 项小修） |
| 审查包同步提交（docs-only） | 其后提交 | **仅 `docs/v0.1.3/`，不含任何代码改动** |
> 阈值小提交阶段新增 `STORE_MANAGER_V0_1_3_BACKEND_THRESHOLD_FIX_NOTES.md`，isolated smoke 升级为 **35/0**。

> **口径定死**：
> **口径定死（阈值小提交阶段）**：
> 1. 基础 commit = `68263a1`；首轮初审修订代码 HEAD = `f1f29a4`；
> 2. **阈值小提交代码 HEAD / 本次复审对象 = `2949888`** —— 复审针对到此为止的代码；
> 3. **`2949888` 之后仅 docs-only 提交** —— 只同步本审查包文档（`docs/v0.1.3/`），不改业务代码；
> 4. `GIT_LOG.txt` 已用 `[CODE]/[DOCS]` 逐行标注，`2949888` 标为 `★代码HEAD/本次复审对象`。
> 三处（GIT_LOG / README / CODE_REVIEW_PACKAGE）口径完全一致：**代码看 `2949888`**，其余为文档同步。

### 审查材料留存边界（merge 前必须移除）
- `docs/v0.1.3/` 下的 **docx 文档、zip 审查包、patch/smoke 等审查材料仅在 review 阶段保留**；
- **未来 merge 前必须移除，不允许合入主分支**（主分支只进业务代码与必要的精简文档，不合入审查过程产物 / 二进制 zip / docx）。
