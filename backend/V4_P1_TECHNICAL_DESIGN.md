...
- **A台主工作台入口 = `POST /api/compose`**（文字+图片→多段15s→后端自动拼接→一个成型母视频）。`POST /api/a/generate` 仅作**底层单段生成/技术备用接口**，不作主入口。
- **B台 `/api/b/batch-generate` 当前已是 require_auth**，P0 需要显式传 `sources`。P1 标准字段改为 **`source_video_ids`**，并按「会话源池优先」三层优先级选源；为空时后端才 fallback。
- **`videos` 表当前没有时长字段** → 实现「源视频时长≥30s」过滤必须新增列。**统一命名 `duration_seconds`**（DB 列 + API 返回字段同名，单位秒）。（注意：`cost_records.duration` 是既有的成本时长列，与此无关，不改名。）
...
### P1-A: `videos` 增加 `duration_seconds`（本轮做，必需）
```sql
-- 原因：P1 B台门槛「源视频时长≥30s」需要后端可判断时长；当前 videos 无时长列。
-- 命名统一：DB 列名 = duration_seconds，API 返回字段名 = duration_seconds（同名，单位秒）。
-- 变更：
ALTER TABLE videos ADD COLUMN duration_seconds FLOAT;   -- 单位：秒；NULL=时长未知

-- 数据迁移（回填存量视频时长）：对 storage_status='active' 且本地文件存在的视频，
--   用 ffprobe 探测后 UPDATE。脚本 backend/tasks/backfill_duration.py（仅读本地文件，不调火山）。
--   伪：for v in active videos:
--          d = ffprobe(local_path(v))            # 成功 → UPDATE videos SET duration_seconds=d
--          if d is None: leave NULL              # 失败 → 保持 NULL（视为"时长未知"）
-- 业务约定：duration_seconds 为 NULL 的视频【不计入合格源视频】（B台不选它）。

-- 回滚：
ALTER TABLE videos DROP COLUMN duration_seconds;   -- SQLite<3.35 不支持 DROP COLUMN，则保留空列即可（不影响旧逻辑）
```
...
### 迁移执行顺序（生产/staging）
1. 备份 DB 文件。
2. 执行 `ALTER TABLE videos ADD COLUMN duration_seconds FLOAT;`
3. **必须**运行回填脚本 `python -m tasks.backfill_duration`（ffprobe 扫存量本地文件写 `duration_seconds`）。回填失败的视频保持 NULL（视为「时长未知」，不计入合格源）。
4. 全新库无需任何 ALTER（`create_all` 建表即含 `duration_seconds` —— 模型已声明）。

> ⚠️ 回流层三表 `workflow_runs/video_feedback_signals/knowledge_candidates` 与 `tasks.run_id`、`videos` 既有 P0 列若生产尚未建，请先按 `BACKEND_V4_PAGE_REDESIGN_P0_REPORT.md` §三补齐，再做本 P1 的 `duration_seconds`。

---

## 3. API 完整规格（变更 + 关键现存）

### 3.1 `POST /api/b/batch-generate`（**P1 变更**：source_pool 优先 + 1:10）
- **权限**：`require_auth`（登录即可，不卡管理员）。
- **P1 标准请求体**：
```json
{
  "prompt": "抗衰主题",
  "source_video_ids": [11, 12, 13],
  "auto_ratio": 10,
  "max_outputs": 50,
  "strategy": "mix"
}
```
  ...
  - `source_video_ids`: int[]（**P1 标准字段**；前端按「会话源池优先」三层优先级提交，见下）
  - `auto_ratio`: integer（选填，默认 10，即 1:10）
  - `max_outputs`: integer（选填，默认 50，硬上限 50）
  - `strategy`: string（选填，默认 `mix`）
- **字段兼容**：旧字段 `sources`（数组 `[{source_video_id,count,strategy}]`）**仅兼容 P0，不推荐前端继续用**；后端内部可兼容映射。
- **源选择三层优先级（后端按此解析）**：
  1. **优先**：请求传入的 `source_video_ids`（= 前端本次会话 `current_source_video_ids`，刚上传/刚生成的）。
  2. 用户在「高级选择」手动更换 → 即 `source_video_ids` 改为用户指定 id。
  3. **`source_video_ids` 为空时才 fallback**：后端选本租户最近合格源视频（不盲扫全历史的默认行为已取消，仅作兜底）。
- **合格源判定（硬门槛）**：`type='mother' AND storage_status='active' AND duration_seconds >= 30`。`duration_seconds` 为 NULL（时长未知）**不计入合格源**。
- **产出数量**：`total_outputs = min(len(合格 source_video_ids) * auto_ratio, max_outputs)`（即 3→30、4→40、5+→50）。
- **响应 `data`**：`{ "batch_id": string, "total_outputs": int, "source_count": int }`。
- **错误码**：`2001`（合格源视频<3 / id 非本租户 / 超上限）；`401`（未登录）；`4029`（成本熔断，B台为 0 通常不触发）；`500`（裂变服务异常）。
...
### 3.3 `POST /api/compose`（**P1：A台主工作台默认入口**）
- **权限**：`require_auth`（登录即可）。
- 请求 `{prompt, total_seconds(5-180), resolution, title?, image_file_ids?}` → 切≤15s 多段 → 逐段火山生成 → ffmpeg concat → **一个成型母视频** → `source_type='generated'` 进母视频/源视频陈列面。响应 `{task_id}`；轮询 `GET /api/tasks/{id}`。
- **会花钱**（火山）。配额/试用/订阅/cost_engine 保护**保留**：`ensure_budget` 熔断（4029）、Patch5 试用仅 A台扣减、逐段按秒计费。
- **这是主工作台「🟠 A台·母视频」按钮的唯一调用入口。**

### 3.4 `POST /api/a/generate`（**底层单段生成 / 技术备用接口**，非主入口）
- **权限**：`require_auth`（**现状即如此**；从来不是 require_super_admin，无需改守卫）。
- 请求 `{prompt, title?, duration(4-15), resolution, image_file_id?}`；响应 `{task_id}`；轮询 `GET /api/tasks/{id}`。**会花钱**。
- 用途：单段母视频生成的底层能力；**不作为主工作台默认入口**（主入口用 §3.3 compose）。
- **一句话批量** `POST /api/generate {text}`：超 `max_a_batch`(默认10) → 2001（防误触）。同样为非主入口的技术能力。

### 3.5 `POST /api/uploads/batch`（现存，P0）
- multipart：`files`(可重复，image/video/file)、`texts`(可重复)。返回 `{uploaded:[{file_id,file_name,file_type,file_size,file_url,thumbnail_url,status,video_id,zip_entries?}],failed:[{file_name,reason}]}`。video 自动登记为 `source_type=uploaded` 的 mother。**P1 待办**：上传 video 时同步用 ffprobe 写入 `videos.duration_seconds`（本轮做，见 §5）。前端拿到 `video_id` 后加入会话 `current_source_video_ids`。

### 3.6 `GET /api/videos`（现存）
- 参数 `type=mother|viral|all`、`source_type`、`batch_id`、`store_id`、`source_video_id`、`strategy`、`page`、`page_size`、`sort=created_desc|created_asc`、`include_expired=false`。**P1 待办**：item 增加 **`duration_seconds`** 字段返回（前端据此显示时长、判断 B台门槛）。
...
|----|----|----|----|
| `POST /api/compose`（**A台主入口**） | `require_auth` | **不变** | A台主工作台默认入口；前端去掉「联系管理员」卡控 + 加费用确认即可，后端无守卫变更 |
| `POST /api/a/generate`（底层备用） | `require_auth`（登录即可） | **不变** | 非主入口，单段生成底层能力 |
| `POST /api/generate`（一句话A台） | `require_auth` | 不变 | 同上；保留 `max_a_batch` 防误触 |
| `POST /api/b/batch-generate` | `require_auth` | 不变（**逻辑变更**：source_pool 优先 + 1:10） | 裂变不卡权限；门槛改为「合格源视频≥3 且 `duration_seconds≥30`」业务校验 |
...
video.duration_seconds = dur         # 失败=None → "时长未知"，不计入合格源
```
A台 compose/单段落库时同样写 `video.duration_seconds`（单段=duration 参数；compose=total_seconds）。
...
```python
# orchestrator.submit_b_batch（P1 改造：source_pool 优先 + 1:10）
def submit_b_batch(db, tenant_id, prompt, source_video_ids=None,
                   auto_ratio=10, max_outputs=50, strategy="mix",
                   sources=None, phone=None):           # sources: P0 旧字段，仅兼容
    # 0) 旧字段兼容：sources -> source_video_ids
    if sources and not source_video_ids:
        source_video_ids = [s["source_video_id"] for s in sources]

    # 1) 选源三层优先级
    if source_video_ids:                                # 优先级 1&2：前端传入（会话源池/用户手选）
        cands = (db.query(Video)
                   .filter(Video.tenant_id==tenant_id,           # 必须本租户（防越权）
                           Video.id.in_(source_video_ids),
                           Video.type=='mother',
                           Video.storage_status=='active',
                           Video.duration_seconds != None,        # 时长未知不计入
                           Video.duration_seconds >= 30)          # 硬门槛 30s
                   .all())
    else:                                               # 优先级 3：fallback 最近合格历史源
        cands = (db.query(Video)
                   .filter(Video.tenant_id==tenant_id, Video.type=='mother',
                           Video.storage_status=='active',
                           Video.duration_seconds != None, Video.duration_seconds >= 30)
                   .order_by(Video.created_at.desc()).all())

    # 2) 门槛：合格源 < 3 → 拒绝
    if len(cands) < 3:
        raise ValueError("请至少上传3个时长30秒以上的视频，才能稳定裂变")   # → 路由 2001

    # 3) 1:10，封顶 max_outputs(默认50)：3→30、4→40、5+→50
    total = min(len(cands) * auto_ratio, max_outputs)
    plan, remaining = [], total
    for v in cands:
        if remaining <= 0: break
        take = min(auto_ratio, remaining)
        plan.append({"source_video_id": v.id, "count": take, "strategy": strategy})
        remaining -= take
    ...
    return {"batch_id": batch_id, "total_outputs": total, "source_count": len(cands)}
```
- 路由 `ValueError → Resp(code=2001)`；前端据此弹「裂变门槛弹窗」。
- `total_outputs = min(len(合格 source_video_ids) * auto_ratio, max_outputs)`；3→30、4→40、5+→50。
- **不再默认盲扫全租户历史**：仅当 `source_video_ids` 为空才走 fallback（兜底）。
...
Step 4  执行 DB migration：ALTER TABLE videos ADD COLUMN duration_seconds FLOAT;
Step 4b （**必须**）运行 ffprobe 回填脚本：python -m tasks.backfill_duration
        - 存量视频 duration_seconds 为空 → 不计入合格源视频（B台不会选它）
        - 回填失败的视频保持 NULL，前端显示「时长未知（需重新上传或等待解析）」
Step 5  重启 staging 后端：systemctl restart v4-video-engine-staging
Step 6  验证后端：curl http://127.0.0.1:<port>/health；抽测 /api/b/batch-generate（传 source_video_ids）+ /api/compose
...
2. 恢复 DB：cp meiye_v4_staging.db.bak.<ts> meiye_v4_staging.db
   （或仅回滚列：SQLite 保留空 duration_seconds 列即可，不影响旧逻辑）
...
| 项 | 归属 | 说明 |
|----|----|----|
| `videos.duration_seconds` 列 + 上传/生成写时长 + **强制回填脚本** | **本轮** | B台门槛与展示所需 |
| B台 source_pool 优先 + 1:10（30/40/50） + 门槛 2001 + `source_video_ids` 标准字段 | **本轮** | `submit_b_batch` 改造 + 路由 |
| `/api/videos` 与列表 item 返回 `duration_seconds` | **本轮** | 前端显示时长、判断门槛 |
| 前端删文本入口/蓝色按钮、A台费用确认、取消勾选、维护 `current_source_video_ids` | **本轮（前端）** | 纯前端，后端无改 |
| A台主入口 = `/api/compose`（文字+图片→多段15s→拼接→母视频） | **本轮（复用）** | 底座已具备 |
| B台裂变成片目标 90–120s（ffmpeg `-t`） | **P1-B**（不并入本轮核心） | remixer 引擎参数调整，紧随本轮 |
...
- B台裂变**至少 3 个合格源视频**；合格 = **`duration_seconds >= 30`（硬门槛）**；`<30s` 或 `NULL（时长未知）`**不计入合格源**。
- 合格源不足 3 个：**B台不可用**；前端提示「请至少上传 3 个时长 30 秒以上的视频，才能稳定裂变」。
- **3 源→30 条；4 源→40 条；5 源及以上→最多 50 条**（每源 1:10，`max_outputs` 封顶 50）。
- B台请求 **P1 标准字段 = `source_video_ids`**（会话源池优先；旧字段 `sources` 仅兼容 P0，不推荐）。
- 前端**主流程不再勾选确认**；维护 `current_source_video_ids`，点 B台优先提交它。
- A台**主入口 = `POST /api/compose`**；**取消「联系管理员」卡控**，改为**登录用户可用 + 费用提醒确认弹窗**（文案：「A台会调用火山API生成母视频，具体费用以实际扣费为准。确认继续吗？」，不写固定单价）。
- A台**仍保留余额/试用/订阅/cost_engine 保护**（熔断 4029、试用仅 A台扣减）。
- A台目标：**文字 + 图片 → 多段 15 秒 → 后端自动拼接 → 成型母视频** → 进母视频/源视频陈列面。
- 裂变结果**必须进入裂变视频陈列面**（`type=viral`，0 元标签）。
- 视频与素材**仍走临时存储策略**（裂变 5 天 / 上传 7 天 / 母视频默认长期可手删；到期自动清理）。
- 业务回流**只进候选池（pending）**，**不直接进阿里云正式大库**（审核后 P2 导出）；候选池审核为 **super_admin 平台级**，user/invite_admin 不显示候选池。
- B台成片 **90–120s 混剪时长优化列为 P1-B**，不并入本轮核心。
