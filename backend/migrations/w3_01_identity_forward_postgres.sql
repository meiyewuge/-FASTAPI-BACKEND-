-- DSM W3-01 identity chain forward migration (PostgreSQL dialect).
-- Real identity PKs, BIGINT FKs, BOOLEAN DEFAULT FALSE, TIMESTAMP now() defaults,
-- CHECK constraints, unique mappings, status_epoch triggers, authoritative store
-- registry (DR-01=A), opaque external ids, machine schema version. Idempotent
-- (IF NOT EXISTS). Matches the ORM.
-- NOTE: not executed against a real PostgreSQL in this environment
-- (POSTGRES_RUNTIME_NOT_EXECUTED); reviewed DDL. Apply via psql/CI so the $$
-- trigger-function body is handled correctly.
