-- DSM W3-01 identity chain forward migration (SQLite dialect).
-- Matches the ORM exactly; idempotent (IF NOT EXISTS). Creates the authoritative
-- store registry (DR-01=A), the employee-identity tables, status_epoch triggers
-- (permanent invalidation on any status change), CHECK constraints, unique
-- mappings, opaque external ids, and a machine schema-version row.
-- The app never runs this automatically; apply via the migrator/CI step.

-- Authoritative store registry: one external opaque id per internal store.
CREATE TABLE IF NOT EXISTS dl_store_registry (
    id             INTEGER PRIMARY KEY AUTOINCREMENT,
    public_id      VARCHAR(32) NOT NULL,
    dl_store_id    VARCHAR(128) NOT NULL,
    main_store_id  INTEGER,
    v013_store_id  VARCHAR(128),
    created_at     TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
CREATE UNIQUE INDEX IF NOT EXISTS uq_dl_store_public_id ON dl_store_registry(public_id);
CREATE UNIQUE INDEX IF NOT EXISTS uq_dl_store_dl_store_id ON dl_store_registry(dl_store_id);
CREATE UNIQUE INDEX IF NOT EXISTS uq_dl_store_main_store_id ON dl_store_registry(main_store_id);
CREATE UNIQUE INDEX IF NOT EXISTS uq_dl_store_v013_store_id ON dl_store_registry(v013_store_id);
...
CREATE TABLE IF NOT EXISTS dl_store_member_binding (
    ...
    member_public_id  VARCHAR(32) NOT NULL,
...
INSERT OR IGNORE INTO dl_identity_schema_meta (version) VALUES ('dsm-w3-01-r1');
