...
CDN 作 fallback。本地副本永不过期，根治「下载/播放失效」。按 mother/viral 分目录。

部署（ECS）：
- STORAGE_ENABLED=true
- STORAGE_DIR=/opt/v4-video-engine/storage/videos
- nginx serve： STORAGE_BASE_URL=https://video.beautypeaceai.com/static/videos
"""
...
def download_and_store(video_id: int, cdn_url: str, subdir: str = "", timeout: float = 60.0) -> dict:
    """下载 CDN mp4 到本地（subdir: mother/viral）。返回 {cdn_url, local_path, local_url, ok}。
    失败不抛，回退 CDN。"""
