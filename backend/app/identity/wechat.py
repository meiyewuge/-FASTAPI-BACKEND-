...
Secret handling (corrected R1b): the WeChat code2session protocol REQUIRES
WECHAT_APP_SECRET to be sent to WeChat as an HTTPS query parameter
(api.weixin.qq.com over TLS); that transmission is unavoidable. Our guarantee is
narrower and accurate: the secret never enters the FRONTEND, our business LOGS,
EXCEPTION messages, the DATABASE, or GIT, and is read only from Settings. The raw
code, session_key and full openid are never logged.
"""
